...
class VmsClient:
    ...
    def __init__(self, spec: VmsSpec, *, dry_run: bool = False) -> None:
        ...
        self._address = spec.address  # used by the DE-API helpers below
    ...
    # ── DataEngine REST API ─────────────────────────────────────────────
    # vastpy auto-routes attribute access to `/api/latest/<resource>/`, but
    # the DataEngine endpoints live at `/api/dataengine/...` (catalog point
    # #1). These helpers bypass vastpy and POST directly with `requests`,
    # using tenant-admin auth + `X-Tenant-Name` header (catalog "Auth"
    # section: tenant-scoped requires this combo).
    ...
    def _de_api_request(
        ...
        resp = requests.request(
            ...
            auth=(tenant_admin_user, tenant_admin_password),
            headers={"X-Tenant-Name": self._tenant_name},
    ...
    def _de_api_list(
        ...
        # DE-API list endpoints are paginated: {results: [...], count, next, ...}
        return resp.get("results", []) if isinstance(resp, dict) else list(resp)
    ...
    def register_de_mtls_credential(
        ...
        """Idempotent: returns guid of existing or newly-created mTLS cred.

        POST `/api/dataengine/mtls-authentication-credentials/` with the 3
        certs base64-encoded (catalog A.4).
        """
    ...
    def register_de_k8s_cluster(
        ...
        """Idempotent: returns vrn of existing or newly-created cluster.

        POST `/api/dataengine/kubernetes-clusters/` (catalog A.3). Side-effect:
        synchronously creates a `VastTenant` CR on the K8s cluster — if one
        already exists in any state (incl. `Deleting` w/ 300s op delay), the
        POST will 400 with "Failed to provision telemetries resources" and
        we surface that as a RuntimeError. Recovery procedure documented in
        the catalog A.3 section.
        """
        existing = self._de_api_list(
            "kubernetes-clusters/",
            tenant_admin_user=tenant_admin_user,
        ...
        for c in existing:
            if c.get("name") == name:
                return c.get("vrn") or f"vast:dataengine:kubernetes-clusters:{name}"
        if self._dry_run:
            return f"<dry-run-cluster-vrn:{name}>"
        body = {
            "name": name,
        ...
        return created.get("vrn") or f"vast:dataengine:kubernetes-clusters:{name}"
    ...
    def register_de_container_registry(
        ...
        """Idempotent: returns guid of existing or newly-created registry.

        POST `/api/dataengine/container-registries/` (catalog A.5). Uses VRN
        (resource path) to reference the cluster, NOT GUID.
        """
        existing = self._de_api_list(
        ...
        for r in existing:
            if r.get("name") == name:
                return r.get("guid", "")
        if self._dry_run:
            return f"<dry-run-registry-guid:{name}>"
        body: dict[str, Any] = {
            "name": name,
            "url": url,
