...
│   │   ├── cli.py                     # entry: `vastde-orch validate|enable|apply|status|...`
│   │   ├── config/
...
├── scripts/
│   ├── setup_tenant.py                # cluster-admin tenant bootstrap (tenant + identity + DE policy)
...
│   ├── tenant-setup.example.yaml      # input config for `scripts/setup_tenant.py`
...
| You want to… | Use |
|---|---|
| **Bootstrap a new tenant** from scratch (tenant + group + bucket-owner + role + manager + VIP pool + view policies + DE identity policy) | `scripts/setup_tenant.py` |
| Find a free IP range in a subnet before creating a VIP pool | `scripts/list_vippools.py` |
| Deploy DataEngine end-to-end on one tenant, **today** | **Minimal schema** + `scripts/test_minimal_enable.py` |
| Deploy DataEngine via the official `vastde-orch enable` CLI (full Stage A pipeline + interactive prompts) | **Full schema** + `vastde-orch enable` |
...
`scripts/setup_tenant.py` creates everything a tenant needs on the VAST cluster
*before* DataEngine can be enabled — driven by a small declarative YAML. The
9-step flow mirrors the VAST KB "Configure Prerequisites" doc + the new
"Provisioning User Access and Permissions for DataEngine" doc (committed at
`docs/provision-user.pdf`).

```bash
cp sample/tenant-setup.example.yaml my-tenant.yaml   # edit the names + IDs
python scripts/setup_tenant.py -c my-tenant.yaml --plan   # dry-run
python scripts/setup_tenant.py -c my-tenant.yaml          # apply
```
...
8. **DataEngine identity policy** — matches the KB doc verbatim (Sids `DataengineTablesAccess` + `DataEngineDefault`), bound to the DE group. Named `<tenant>-de-write` because the KB's `data-engine-<tenant>` name is reserved by VMS (POST returns 403)
9. **(Opt-in) `AllowAllTabular` bind** — broader S3 + Kafka access; off by default
...
**VAST cluster** (pre-existing, cluster-admin one-time — or run `scripts/setup_tenant.py` to create all of this in one shot):
...
| [`docs/provision-user.pdf`](docs/provision-user.pdf) | VAST KB: "Provisioning User Access and Permissions for DataEngine" — the source-of-truth for the identity policy that `scripts/setup_tenant.py` step 8 implements. Lists every `dataengine:*` action. |
...
- **Users/groups are tenant-scoped via the tenant's auto-created local provider**
  ...
  `setup_tenant.py` reads `local_provider_id` off the tenant record and uses
  it for both group and bucket-owner user.
- **The DataEngine identity policy name `data-engine-<tenant>` is reserved by VMS** —
  ...
  group" checkbox; the REST `enable` flow can't trigger it. `setup_tenant.py`
  creates an equivalent policy (identical document) under `<tenant>-de-write`.
